...
If no GPU is available you can comment out the related `runArgs` (*gpus*, *runtime*) in the [`devcontainer.json`](.devcontainer/devcontainer.json) to run on cpu only:
```json
{
    "name": "ros2_humble",
